...
class Programs {
	...
	async stop_all_programs() {
		gadapter.log.info(`Stopping all programs: ${stringify(this.proglist)}`);
	...
	*/
	...
	async handlequeue() {
		...
		p.start(await this.weather.condition(gadapter.config.numdays, gadapter.config.condition_override));
...
class Program {
	...
	async nextZone(id) {
		...
		if (this.isParallel()) {
			...
			for (const z of zl) {
				if (z.running) {
					gadapter.log.debug(`nextZone parallel: zone ${z.name} still running`);
					return true; // wait until all zones stoped
				}
			}
			this.active = 0;
			this.running = false;
